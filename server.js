...
.stats,.form-grid,.providerTextGrid{grid-template-columns:1fr}
...
res.send(`
<h2 style="font-family:Arial;text-align:center;margin-top:80px;">Locked Admin Dashboard</h2>
<p style="text-align:center;font-family:Arial;">Add your admin token to open this page.</p>
`);
return false;
}
return true;
...
function providerTextButtons(lead) {
return `
<div class="quickTextBox">
<div class="quickTitle">⚡ Instantly Text Job To Provider</div>
<div class="providerTextGrid">
<a class="providerTextBtn maxBtn" href="${smsLink(providers["Max"], lead)}">💬 Text Max</a>
<a class="providerTextBtn drehBtn" href="${smsLink(providers["Dreh"], lead)}">💬 Text Dreh</a>
<a class="providerTextBtn teeBtn" href="${smsLink(providers["Tee"], lead)}">💬 Text Tee</a>
<a class="providerTextBtn robynBtn" href="${smsLink(providers["Robyn"], lead)}">💬 Text Robyn</a>
<a class="providerTextBtn chrisBtn" href="${smsLink(providers["Car Key Chris"], lead)}">💬 Text Chris</a>
</div>
</div>`;
}
...
<div class="info"><b>Duration:</b> ${lead.duration || ""}</div>
<div class="info"><b>Score:</b> ${lead.lead_score || ""}</div>
...
<div class="btn-grid">
<button class="blue" type="submit">💾 Save Pricing</button>
<a class="btn green" href="tel:${cleanPhone(lead.customer_phone)}">📞 Call Customer</a>
</div>
</form>

${providerTextButtons(lead)}
` : ""}
...
if (!leads.length) {
content += `<div class="card"><h2>No leads yet.</h2></div>`;
}

leads.forEach(lead => {
content += leadCard(lead, true);
});
...
let content = `
<div class="top">
<h1>👷 Provider Dashboards</h1>
<p>Share these links with providers. They only see their own assigned jobs.</p>
</div>
<div class="card">
`;
...
<div class="top">
<h1>⚙️ Settings</h1>
<p>Provider text buttons are active.</p>
</div>
<div class="card">
<h2>Providers</h2>
...
if (filter === "new") {
query += ` AND (lead_status IS NULL OR lead_status = 'New')`;
}

if (filter === "completed") {
query += ` AND lead_status = 'Completed'`;
}
...
<p>Only your assigned jobs show here.</p>
</div>
`;

if (!leads.length) {
content += `<div class="card"><h2>No jobs assigned yet.</h2></div>`;
}

leads.forEach(lead => {
content += leadCard(lead, false);
});
...
const leads = result.rows;
const earnings = leads.reduce((sum, l) => {
return sum + (Number(l.job_amount || 0) - Number(l.lead_cost || 0));
}, 0);

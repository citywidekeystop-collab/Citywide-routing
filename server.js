...
body.recording ||
body.recording_url ||
body.recording_player ||
"",

lead_score: String(body.lead_score || "N/A"),

call_status: body.answered === false ? "Missed" : "New",
...
console.log("✅ CALLRAIL LEAD SAVED", saved.id);
...
console.error("❌ CALLRAIL ERROR:", err);
res.status(500).json({
success: false,
error: err.message
});
...
.sidebar{width:240px;background:#0f172a;padding:20px;border-right:1px solid #1e293b}
.logo{font-size:26px;font-weight:bold;margin-bottom:25px}
.nav a{display:block;color:#cbd5e1;background:#111827;text-decoration:none;padding:12px;border-radius:10px;margin-bottom:10px}
...
.card{background:#0f172a;border:1px solid #1e293b;border-radius:16px;padding:20px;margin-bottom:15px}
.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:20px}
.stat{background:#111827;padding:18px;border-radius:16px}
.stat h2{margin:0;color:#38bdf8}
button{border:none;padding:10px 14px;border-radius:10px;font-weight:bold;cursor:pointer;margin:4px}
.accept{background:#16a34a;color:white}
.booked{background:#7c3aed;color:white}
.paid{background:#f59e0b;color:black}
.decline{background:#64748b;color:white}
...
@media(max-width:800px){.layout{display:block}.sidebar{width:100%}}
</style>
</head>
<body>
<div class="layout">
...
<p>CallRail leads, provider routing, notes, and PostgreSQL storage.</p>
...
<div>
<input id="search" placeholder="Search leads..." oninput="renderLeads()">
<select id="filter" onchange="renderLeads()">
...
const filtered = leadsData.filter(function(lead){
const text = JSON.stringify(lead).toLowerCase();
const matchSearch = text.includes(search);
const matchFilter = !filter || lead.lead_status === filter;
return matchSearch && matchFilter;
});

if(filtered.length === 0){
wrap.innerHTML = '<div class="card"><h2>No leads found</h2></div>';
return;
}

wrap.innerHTML = filtered.map(function(lead){
return '<div class="card">' +
'<h2>📞 ' + (lead.customer_phone || "Unknown") + '</h2>' +
...
'<br><select onchange="assignProvider(' + lead.id + ', this.value)">' +
...
<div class="card">
<h1>Calls</h1>
<p>CallRail calls and recordings will show here.</p>
<a href="/admin/leads" target="_blank">View raw call leads</a>
</div>
`));
});
